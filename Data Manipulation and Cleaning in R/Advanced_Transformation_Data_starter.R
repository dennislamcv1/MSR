...
item_data <- read_csv("  ")  # read_csv() from the readr package reads the CSV file into a data frame.
...
avg_rating_by_category <- item_data %>%
  __________(Category) %>%
  __________(avg_rating = __________(Rating, na.rm = TRUE))
...
total_revenue_by_category <- item_data %>%
  __________(Category) %>%
  __________(total_revenue = __________(revenue, na.rm = TRUE))
